...
- **无需安装 Docker 或 go 环境**：直接使用单文件 EXE 或 go 脚本，开箱即用。
- **无依赖 EXE 执行**：编译为独立 EXE 文件，无需安装 go 环境，无需安装 Docker 环境，直接在 Releases 下载就能直接使用。
...
前往 [Releases](https://github.com/guquan-lengyue/docker_pull_tar/releases) 页面，下载 `DockerPull.exe`，无需安装任何依赖，直接运行。


### 通过 Git 克隆

```bash
git clone https://github.com/guquan-lengyue/docker_pull_tar.git
```

### 基本用法

```bash
go run main.go [镜像名称] [架构] [仓库地址]
```
...
```bash
go run main.go -i alpine -a arm64 -r 1ms.run --debug
```
...
如有任何问题或建议，请通过 [GitHub Issues](https://github.com/guquan-lengyue/docker_pull_tar/issues) 提出。

## 为什么选择这个工具？

- **无需安装 Docker 或 go**：直接运行 EXE 文件，适合内网环境。
- **速度快**：国内镜像源加速，下载更快。
